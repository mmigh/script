...
(function()
    ...
    Interface.BlurManager = blurEffect
        ...
        spawn(function()
            pcall(function()
                LocalPlayer.PlayerScripts:WaitForChild("NewIslandLOD", 9999):Destroy()
    ...
    local RawMobRegions = game:HttpGet("https://raw.githubusercontent.com/kophaitoi/thangchonguvl/refs/heads/main/caideogivay") 
    ...
    function TweenController.Create(Position)
        ...
        if TweenTargetPosition and (Position.Position - TweenTargetPosition).Magnitude < 10 then return end
        TweenTargetPosition = Position.Position
    ...
    function CheckSpecialMicrochip() 
        for _, v in { LocalPlayer.Character:GetChildren(), LocalPlayer.Backpack:GetChildren() } do
            for _, v2 in v do 
                if v2.Name == 'Special Microchip' then 
                    return v2
                end 
            end 
        end 
    ...
    FunctionsHandler.RaidController:RegisterMethod("Start", function()
        ...
        if not CurrentIsland then 
            print("[RAID DEBUG] No island - buying chip phase")
            SetTask("MainTask", "Auto Raid - Buying Chip - " .. FunctionsHandler.RaidController:Get("CurrentChip")) 
            
            print("[RAID DEBUG] HasChip:", ScriptStorage.Tools["Special Microchip"] ~= nil)
            if not ScriptStorage.Tools["Special Microchip"] then 
                ...
                print("[RAID DEBUG] RaidFruit:", cRaidFruit and cRaidFruit.Name or "nil")
            ...
            print("[RAID DEBUG] RootRaidIsland:", RootRaidIsland, "SeaIndex:", SeaIndex)
            if not RootRaidIsland then return end
            FunctionsHandler.LocalPlayerController.Methods.EquipTool:Call("Special Microchip")
            
            local inMap = workspace.Map:FindFirstChild(RootRaidIsland)
            local inWorkspace = workspace:FindFirstChild(RootRaidIsland)
            print("[RAID DEBUG] inMap:", inMap ~= nil, "inWorkspace:", inWorkspace ~= nil)

            if not inMap and not inWorkspace then 
                print("[RAID DEBUG] Island not found anywhere - tweening to FakeIslands")
                task.wait(1)
                local target = game:GetService("ReplicatedStorage").FakeIslands:FindFirstChild(RootRaidIsland):GetModelCFrame()
                TweenController.Create(target)
                repeat task.wait(0.5) 
                    print("[RAID DEBUG] waiting arrive FakeIsland, dist:", CaculateDistance(target))
                until CaculateDistance(target) < 100 or not target
                return
            end 
            
            local hasRaidSummon2 = inMap and inMap:FindFirstChild('RaidSummon2')
            print("[RAID DEBUG] hasRaidSummon2:", hasRaidSummon2 ~= nil)
            if not hasRaidSummon2 then 
                print("[RAID DEBUG] No RaidSummon2 - tweening to map island")
                task.wait(1)
                local target = workspace.Map:FindFirstChild(RootRaidIsland):GetModelCFrame()
                TweenController.Create(target)
                repeat task.wait(0.5) 
                    print("[RAID DEBUG] waiting arrive island, dist:", CaculateDistance(target))
                until CaculateDistance(target) < 100 or not target
                return
            end 
            
            print("[RAID DEBUG] Clicking RaidSummon2")
    ...
    spawn(function() 
        ...
        for _, Connection in getconnections(game.Players.LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Main")
                                        :WaitForChild("HUDButtonBar"):WaitForChild("Settings"):WaitForChild("Buttons"):WaitForChild("FastModeButton")
                                        .Activated) do
    Connection.Function()
end
