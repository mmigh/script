pcall(function()
    ...
    -- ============================================================
    -- [9] STORE FRUIT & CHECK BACKPACK LOGIC
    -- ============================================================
    local FruitCache = {
        mapFruits = {},
        mapFruitsCount = 0,
    ...
    -- ============================================================
    -- [12] RAID SYSTEM - FIXED CLICK JOIN RAID
    -- ============================================================
    ...
    -- TÌM NÚT RAID - CẢI THIỆN
    function Raid:FindButton()
        -- Cách 1: Tìm trong Map
            ...
            for _, isl in ipairs(map:GetChildren()) do
                ...
                if rs then
                    ...
                    if btn then
                        ...
                        if main then
                            local cd = main:FindFirstChild("ClickDetector")
        ...
        -- Cách 2: Tìm trong Workspace
        for _, child in ipairs(Workspace:GetDescendants()) do
            if child:IsA("ClickDetector") and child.Parent and child.Parent.Name == "Main" then
                local parent = child.Parent.Parent
                if parent and parent.Name == "Button" and parent.Parent and parent.Parent.Name == "RaidSummon2" then
                    return child
                end
            end
        end
    ...
    -- CLICK NÚT RAID - CẢI THIỆN VỚI NHIỀU CÁCH
    function Raid:ClickRaidButton()
        local btn = self:FindButton()
        if not btn then
            print("[RAID] ❌ Không tìm thấy nút raid!")
            return false
        end
        
        print("[RAID] 🔍 Đã tìm thấy nút raid!")
        
        -- Cách 1: fireclickdetector
        if fireclickdetector then
            pcall(function()
                fireclickdetector(btn)
                print("[RAID] ✅ Click bằng fireclickdetector!")
                return true
            end)
        end
        
        -- Cách 2: Click chuột ảo
        pcall(function()
            local pos = btn.Parent and btn.Parent.Position
            if pos then
                local cam = Workspace.CurrentCamera
                if cam then
                    local screenPos, onScreen = cam:WorldToViewportPoint(pos.Position)
                    if onScreen then
                        local x, y = screenPos.X, screenPos.Y
                        mouse = game.Players.LocalPlayer:GetMouse()
                        if mouse then
                            mouse.Move(mouse, Vector2.new(x, y))
                            task.wait(0.1)
                            mouse.Button1Down(mouse)
                            task.wait(0.1)
                            mouse.Button1Up(mouse)
                            print("[RAID] ✅ Click bằng chuột ảo!")
                            return true
                        end
                    end
                end
        ...
        -- Cách 3: Invoke remote (nếu có)
        pcall(function()
            if CommF then
                CommF:InvokeServer("StartRaid", Config.SelectRaid)
                print("[RAID] ✅ Click bằng remote!")
                return true
            end
        end)
        
        print("[RAID] ❌ Tất cả cách click đều thất bại!")
        return false
    end

    function Raid:Start(canStartWithFruit)
        if not CommF then 
            print("[RAID] ❌ Không có CommF!")
            return false 
        end
        if os.time() - lastRaidStart < 10 then 
            print("[RAID] ⏳ Cooldown 10s...")
            return false 
        end
        if raidInProgress then 
            print("[RAID] ⏳ Đang start raid...")
            return false 
        end

        local hasChip = self:CheckBackpackChipOnce()
        print("[RAID] 🔍 Chip: " .. tostring(hasChip) .. " | Fruit: " .. tostring(canStartWithFruit))
        
        -- Nếu có fruit và chưa có chip -> mua chip
        if canStartWithFruit and not hasChip then
            if not BoughtChipForCurrentRaid then
                print("[RAID] 💰 Đang mua chip...")
                pcall(function()
            ...
            print("[RAID] 💰 Sau khi mua chip: " .. tostring(hasChip))
        end

        if not hasChip and not canStartWithFruit then 
            print("[RAID] ❌ Không có chip hoặc fruit!")
            return false 
        end
        ...
        -- Di chuyển đến vị trí start raid
        print("[RAID] 🚀 Di chuyển đến vị trí raid...")
        TweenCtrl:CreateAndWait(RAID_START_CF)
        task.wait(0.5)

        -- Click nút raid
        print("[RAID] 🖱️ Đang click nút raid...")
        local success = self:ClickRaidButton()
        
        if success then
            self.IslandIdx   = 1
            self.StartIsland = nil
            FruitCache.cacheValid = false
            print("[RAID] ✅ Raid đã được bắt đầu!")
            task.wait(2)
            raidInProgress = false
            return true
        end
        
        print("[RAID] ❌ Không thể click nút raid!")
        raidInProgress = false
        return false
    end
    ...
    task.spawn(function()
        ...
        while task.wait(0.15) do
            ...
            pcall(function()
                ...
                -- ƯU TIÊN 1: NHẶT FRUITS TRÊN MAP
                    ...
                    if Raid:InRaid() then
                        ...
                        if hasChip or hasFruit then
                            if os.time() - lastBuy > 10 then
                                ...
                                print("[MAIN] 🚀 Gọi Raid:Start với hasFruit=" .. tostring(hasFruit))
    ...
    print("║  ✅ FIXED: Click Join Raid with multiple methods     ║")
